  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      const { data: loan, error: loanErr } = await supabase
        .from("peminjaman")
        .select(`transaksi_id, tanggal_peminjaman, user_id, users (nama_lengkap)`)
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.conditionContainer}>
              {conditions.map((item) => (
                <TouchableOpacity
                  key={item.value}
                  style={[styles.conditionChip, condition === item.value && styles.conditionChipActive]}
                  onPress={() => setCondition(item.value)}
                >
                  <Text style={[styles.conditionText, condition === item.value && styles.conditionTextActive]}>
                    {item.label}
                  </Text>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  scanButton: { backgroundColor: "#fff", borderRadius: 20, padding: 30, alignItems: "center", borderStyle: "dashed", borderWidth: 2, borderColor: "#007AFF", marginBottom: 20 },
  scanIconCircle: { width: 70, height: 70, borderRadius: 35, backgroundColor: "#EBF5FF", justifyContent: "center", alignItems: "center", marginBottom: 10 },
  scanBtnText: { color: "#007AFF", fontWeight: "600" },
  loanInfoCard: { backgroundColor: "#E8F5E9", padding: 20, borderRadius: 20, marginBottom: 20, borderWidth: 1, borderColor: "#34C759" },
  cardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  cardTitle: { marginLeft: 8, fontWeight: 'bold', color: '#2E7D32' },
  ...
  borrowerName: { fontSize: 13, color: '#444', fontStyle: 'italic' },
  ...
  conditionContainer: { flexDirection: "row", justifyContent: "space-between", marginBottom: 20 },
  conditionChip: { flex: 1, marginHorizontal: 4, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: "#EEE", backgroundColor: "#F8F9FA", alignItems: "center" },
  conditionChipActive: { backgroundColor: "#007AFF", borderColor: "#007AFF" },
  conditionText: { fontSize: 12, fontWeight: "600", color: "#666" },
  conditionTextActive: { color: "#fff" },
  inputWrapper: { backgroundColor: "#F8F9FA", borderRadius: 10, paddingHorizontal: 12, borderWidth: 1, borderColor: "#EEE", marginBottom: 20 },
  input: { paddingVertical: 12, textAlignVertical: 'top', minHeight: 80 },
